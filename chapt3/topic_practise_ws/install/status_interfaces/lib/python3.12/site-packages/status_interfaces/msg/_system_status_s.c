...
bool status_interfaces__msg__system_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // net_cent
    PyObject * field = PyObject_GetAttrString(_pymsg, "net_cent");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->net_cent = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // net_recv
...
PyObject * status_interfaces__msg__system_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // net_cent
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->net_cent);
    {
      int rc = PyObject_SetAttrString(_pymessage, "net_cent", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
